...
const Detail: FC<propsType> = ({
  ...
  return (
    <div className="detailPage">
      <h2 className="title">
        {(articleDetail.detail && articleDetail.detail.title) || ''}
      </h2>
      <div className="message">
        <IconText
          className="messageItem"
          icon={FieldTimeOutlined}
          text={(articleDetail.detail && articleDetail.detail.createTime) || ''}
        ...
        <IconText
          ...
          text={
            ...
            articleDetail.detail.tagValue &&
            getText(articleDetail.detail.tagValue)
          }
        ...
        <IconText
          className="messageItem"
          icon={EyeOutlined}
          text={`浏览量${articleDetail.detail && articleDetail.detail.look}`}
          key="list-vertical-message"
        />
      </div>
      {/*<MarkDownView source={articleDetail.detail && articleDetail.detail.text || ''} />*/}
      <div className={`styles.txtContent ${quill['ql-snow']}`}>
        <div
          className={quill['ql-editor']}
          dangerouslySetInnerHTML={{
            __html: (articleDetail.detail && articleDetail.detail.text) || '',
          }}
        ></div>
      </div>
      <Message
      ...
      <div className="endBox">
        ...
        <Divider dashed className="divider">
          如果有任何疑问,欢迎和我进行交流
          <EmailIcon style={{ color: '#b9b9b9' }} /> hh_todd@163.com
        </Divider>
      </div>
    </div>
  );
